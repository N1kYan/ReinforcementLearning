...
\\\\
\parTitle{Critic Update} For the critic we use a compatible function approximation of the advantage function. The use of a compatible function approximation $A_w(s, a)$ to estimate $A(s, a)$ \citep{sutton2000policy} is a key aspect of the eNAC algorithm. By definition, a compatible function approximation has the property that it's gradient can be expressed in terms of the policy. This means, that we can express the advantage function by taking the derivative w.r.t. the policy and multiplying it by $w$:
...
\\\\
\parTitle{Actor Update} The reason why we update the policy parameters in the direction of the critic parameters, is the utilization of a compatible function approximation to estimate the advantage function (equation \ref{eq:adv2}). With this the natural policy gradient from equation \ref{eq:3} simplifies in the following way:
